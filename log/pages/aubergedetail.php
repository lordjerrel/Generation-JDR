...
//affichage de l'auberge
	...
	$output.= '<br />'.'<input type="submit" name="genere" value="Générer la même auberge" />';	$output.= '<input type="button" value="Générer une autre auberge"  OnClick="window.location.href='."'index.php?page=aubergedetail'".'"></form>'.'<p style="font-size:smaller"><i>Vous pouvez compléter la création de votre auberge par la génération de son menu : <a href="https://www.generation-jdr.fr/index.php?page=repas" target="_blank">Générateur de repas</a></i></p>';		
	echo $output;
