...
if(!empty($_POST)){
	...
	foreach ($_POST as $key=>$val){			
		...
		$output.= '<input type="submit" name="genere" value="Générer les mêmes objets non-magique" />';	$output.= '<input type="button" value="Générer d\'autres objets non-magique"  OnClick="window.location.href='."'index.php?page=objnm'".'"></form>';		
	echo $output;
}
else {
	$output ='';
