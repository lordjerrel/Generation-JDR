...
switch (rand(0,3)) {
	...
	$aideprorio = $aidemasnd[rand(0,count($aidemasnd)-1)].' '.creernomevo(rand(1,9),1);
	break;
	case 3: //nom différent fem
	$aideprorio = $aidefemnd[rand(0,count($aidefemnd)-1)].' '.creernomevo(rand(1,9),2);
	break;
...
$output =' ';
$output.='<h2>'.$nom.'</h2>';
$output.=$nom." est un".$type[rand(0,count($type)-1)]." ".$divers1[rand(0,count($divers1)-1)]." ".$qualif[$qualite];
...
else if ($qualite==1) $output.='cuistot n\'a pas de recette favorite, mais la cuisine y est correct.<br/>';
